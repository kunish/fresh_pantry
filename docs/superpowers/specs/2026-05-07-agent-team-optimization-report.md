...
| lib/screens/custom_recipe_form_screen.dart:111 | high | theme-inconsistency | 多处节标题用 GoogleFonts.manrope(18,w700) 而非 textTheme。 [blocked by HIGH item in same file] | 统一为 textTheme.titleLarge。 | LOW | ux | auto-approved | done |
...
| lib/screens/custom_recipe_form_screen.dart:550 | high | theme-inconsistency | _CoverImageHero 渐变纯黑硬编码,未走 colorScheme。 [blocked by HIGH item in same file] | 抽 const 或用 AppColors 衍生。 | LOW | ux | auto-approved | done |
...
| lib/screens/dashboard_screen.dart:99 | high | allocation-in-build/theme-inconsistency | [perf] 每次 build 都 new 一堆 GoogleFonts/BoxDecoration/BorderRadius/withValues 颜色。 ; [ux] 问候语用 GoogleFonts.plusJakartaSans 写死 fontSize:30,绕过 textTheme.displayMedium(28)。 [blocked by HIGH item in same file] | [perf] 抽 static const TextStyle 或顶层常量。 ; [ux] 改用 Theme.of(context).textTheme.displayMedium。 | LOW | perf,ux | auto-approved | done (ux: textTheme.displayMedium; perf: 留待 Batch 13) |
| lib/screens/dashboard_screen.dart:154 | high | theme-inconsistency | "紧急关注"用 urgentAttentionBackground.withValues(alpha:0.5) 直接调,层级混乱。 | 直接使用 errorContainer 或新增 token urgentSurface。 | HIGH | ux | approved | done |
| lib/screens/dashboard_screen.dart:177 | high | theme-inconsistency | "紧急关注"内嵌 GoogleFonts.plusJakartaSans 与 textTheme.titleLarge(20) 重复。 [blocked by HIGH item in same file] | 替换为 Theme.of(context).textTheme.titleLarge。 | LOW | ux | auto-approved | done |
...
| lib/screens/dashboard_screen.dart:295 | high | theme-inconsistency | "存储概况"等节标题字号 22 在 textTheme 不存在。 [blocked by HIGH item in same file] | 统一节标题为 textTheme.headlineSmall 或 titleLarge。 | LOW | ux | auto-approved | done |
...
| lib/screens/inventory_screen.dart:144 | high | theme-inconsistency | 标题使用 GoogleFonts.plusJakartaSans fontSize:32 与 displayLarge(32) 重复定义。 [blocked by HIGH item in same file] | 改用 textTheme.displayLarge。 | LOW | ux | auto-approved | done |
...
| lib/screens/shopping_list_screen.dart:71 | high | theme-inconsistency | fontSize:32 重复 displayLarge。 [blocked by HIGH item in same file] | 改用 textTheme.displayLarge。 | LOW | ux | auto-approved | done |
...
| lib/theme/app_theme.dart:58 | high | theme-inconsistency | 缺少集中的 spacing/radius/elevation token,所有 screen 自由使用魔术数字。 | 新增 AppSpacing/AppRadius 常量类。 | HIGH | ux | approved | done |
...
| lib/widgets/common/category_chips.dart:138 | high | theme-inconsistency | 直接 TextStyle(fontSize:13) 不走 textTheme。 | 用 Theme.of(context).chipTheme 或 textTheme.labelLarge。 | LOW | ux | auto-approved | done |
...
| lib/widgets/common/search_overlay.dart:69 | high | theme-inconsistency | Colors.black.withValues 多处用作阴影,而其他卡片使用 AppColors.onSurface.withValues。 [blocked by HIGH item in same file] | 统一阴影色。 | LOW | ux | auto-approved | done |
...
| lib/widgets/dashboard/alert_card.dart:69 | high | theme-inconsistency | name 用 GoogleFonts.manrope(fontSize:17) 偏离 textTheme。 | 用 titleMedium(16) 或新增 token。 | LOW | ux | auto-approved | done |
| lib/widgets/dashboard/alert_card.dart:99 | high | theme-inconsistency | storageTag 内联硬编码 fontSize:9/letterSpacing:0.8。 | 统一 micro-label token。 | LOW | ux | auto-approved | done |
...
| lib/widgets/dashboard/curators_tip_card.dart:43 | high | theme-inconsistency | bottomLabel fontSize:10 letterSpacing:2 不统一。 | 抽 textTheme.labelSmall。 | LOW | ux | auto-approved | done |
...
| lib/widgets/dashboard/recent_addition_item.dart:31 | high | theme-inconsistency | 普通 TextStyle 无 GoogleFonts,字体不一致。 | 全部改用 textTheme。 | LOW | ux | auto-approved | done |
...
| lib/widgets/dashboard/storage_summary_card.dart:43 | high | theme-inconsistency | 系统默认字体 TextStyle(fontWeight:.w700,fontSize:17)。 | 用 textTheme.titleMedium。 | LOW | ux | auto-approved | done |
...
| lib/widgets/inventory/ingredient_card.dart:88 | high | theme-inconsistency | name 用裸 TextStyle 无 GoogleFonts。 [blocked by HIGH item in same file] | 改用 textTheme.titleMedium / Manrope。 | LOW | ux | auto-approved | done |
...
| lib/widgets/shared/freshness_meter.dart:60 | high | theme-inconsistency | 仍使用裸 TextStyle(无 GoogleFonts)。 | 替换为 textTheme.labelSmall + Manrope。 | LOW | ux | auto-approved | done |
...
- 2026-05-08 Batch 4 (主题 token 体系 + textTheme 一致化) 完成: 5 个 commit。新增 `lib/theme/app_spacing.dart`、`lib/theme/app_radius.dart` 集中 spacing/radius token,从 `app_theme.dart` 一并 export 实现零额外 import 渗透。共 ~16 处高频 borderRadius 调用替换为 token (PillChip / app_snackbar / app_dialog / dashboard 5 张卡片 / inventory ingredient_card / common category_chips)。textTheme 一致化覆盖 dashboard / inventory / shopping_list / custom_recipe_form 4 个屏幕 + alert_card / recent_addition_item / storage_summary_card / curators_tip_card / freshness_meter / category_chips / ingredient_card 7 个组件。dashboard "紧急关注" 容器背景由 `urgentAttentionBackground.withValues(0.5)` 切到 `colorScheme.errorContainer` (Unit 4C)。共 12 项 theme-inconsistency + 1 项 allocation-in-build (ux 子项) 标记 done,1 项 perf 子项 (dashboard:99 allocation) 留待 Batch 13。视觉 drift 备忘:dashboard 问候 30→28、节标题 22→20、custom_recipe_form 节标题 18→20、alert_card name 17→16、storage_summary_card name 17→16,均为 1-2 px 字号差,可接受。`flutter analyze` 0 issues,`flutter test` 186/186 通过(基线 173 + Batch 3 引入 11 + Batch 4 新增 2 个 token 测试)。
