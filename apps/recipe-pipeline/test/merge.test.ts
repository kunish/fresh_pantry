...
describe('mergeWithExisting', () => {
  it('imageUrl 既有优先;用量取 fresh;description 黏住;remoteVersion 保留', () => {
    ...
    const { merged, stats } = mergeWithExisting(fresh, existing);
  ...
  it('refreshDescriptions 时才覆盖描述', () => {
    ...
    const { merged } = mergeWithExisting(fresh, existing, { refreshDescriptions: true });
  ...
  it('新菜:remoteVersion 0、clientUpdatedAt/deletedAt null', () => {
    const { merged, stats } = mergeWithExisting([rec({ id: 'b' })], []);
  ...
  it('本轮未触及的既有菜原样保留', () => {
    ...
    const { merged } = mergeWithExisting([rec({ id: 'a' })], existing);
    expect(merged.map((r) => r.id).sort()).toEqual(['a', 'keep']);
  });

  it('输出按 id 稳定排序', () => {
    const { merged } = mergeWithExisting([rec({ id: 'c' }), rec({ id: 'a' })], [rec({ id: 'b' })]);
  ...
  it('更新既有菜时保留 clientUpdatedAt', () => {
    ...
    const { merged } = mergeWithExisting([rec({ id: 'a', name: '新名' })], existing);
...
describe('mergeWithExisting videoUrl', () => {
  ...
  it('既有 videoUrl 优先,不被 fresh 的 null 覆盖', () => {
    const { merged } = mergeWithExisting([baseV({ videoUrl: null })], [baseV({ videoUrl: 'https://b23.tv/x' })]);
    expect(merged[0].videoUrl).toBe('https://b23.tv/x');
  });
  it('既有无 videoUrl 时采纳 fresh 的', () => {
    const { merged } = mergeWithExisting([baseV({ videoUrl: 'https://youtu.be/y' })], [baseV({ videoUrl: null })]);
