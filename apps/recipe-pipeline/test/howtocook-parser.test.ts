...
describe('isTool', () => {
  ...
  it.each([
    '杯子', '一次性透明塑料杯', '利口酒杯', '大号的玻璃杯', '调酒杯', '高球杯',
    '定时器', '分蛋器', '量酒器', '可密封容器', '干净密封玻璃或陶瓷容器',
    '面包机', '打火机', '搅拌工具', '搅拌棒', '圆碟子', '蘸料碟', '蒸鱼盘子',
    '烧烤炉', '烤网', '炭火或燃气', '烘焙纸', '锡箔纸', '秒表', '蒸箱', '汤匙',
  ])('%s 是工具(九轮缺量回填暴露的混入回归)', (s) => {
    expect(isTool(s)).toBe(true);
  });
  it.each([
    '鸡蛋', '西红柿', '盐', '黄瓜',
    // 饮品配料/食材,虽含易误判字仍须保留(冰块/苏打水/网油/迷迭香等)
    '冰块', '打碎的冰块', '冰镇苏打水', '苏打气泡水', '遇水发光冰块',
    '迷迭香', '芦笋', '预制牛排酱汁', '网油',
  ])('%s 不是工具', (s) => {
