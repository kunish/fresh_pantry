...
void main() {
  ...
  testWidgets(
    'SettingsScreen shows household entry row with member count subtitle',
    (tester) async {
      ...
      // Entry row label and subtitle visible on settings screen.
      expect(find.text('家庭共享'), findsWidgets);
      expect(find.textContaining('Kunish Kitchen'), findsWidgets);
      expect(find.textContaining('2 名成员'), findsOneWidget);
      // Inline member rows are gone from settings (now live in HouseholdScreen).
      expect(find.text('member@example.com'), findsNothing);
    },
  );

  testWidgets(
    'SettingsScreen household entry row navigates to HouseholdScreen',
    (tester) async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final gateway = HouseholdGatewayStub(
      ...
      // Tap the household entry row — it should push HouseholdScreen.
      await tester.tap(find.byKey(const Key('household_entry_row')));
      await tester.pumpAndSettle();
