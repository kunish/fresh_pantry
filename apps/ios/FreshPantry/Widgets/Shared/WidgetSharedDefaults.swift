...
public enum WidgetSharedDefaults {
    ...
    public static var suite: UserDefaults? { UserDefaults(suiteName: appGroupID) }

    /// 写当前作用域身份。`into` 可注入测试 suite;默认走共享 suite。
    public static func writeIdentity(householdID: String, clientID: String, into defaults: UserDefaults? = WidgetSharedDefaults.suite) {
        guard let defaults else { return }
        defaults.set(householdID, forKey: householdIDKey)
        defaults.set(clientID, forKey: clientIDKey)
    }

    public static func readHouseholdID(from defaults: UserDefaults? = WidgetSharedDefaults.suite) -> String {
        defaults?.string(forKey: householdIDKey) ?? ""
    }

    public static func readClientID(from defaults: UserDefaults? = WidgetSharedDefaults.suite) -> String {
